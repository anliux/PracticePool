...
- ## 解题思路
  - 思路：字符串
  - 本题的本质是计数器
    - 两层循环：外循环控制行数，内循环遍历上一行生成的字符串
    - 外循环：2-n（包含等于n），n=1的情况直接返回，从2开始
    - 内循环：
      - 从s的[0]开始设置-pre，计数器count，循环从1开始，数与pre相等的个数，直到不再相等
      - 不相等的时候，调整pre为不再相等位置的数字，并将count归为1，重新开始计数
  - 注：本题标签是字符串，考虑用可变字符串StringBuilder
