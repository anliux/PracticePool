...
- [力扣k神题解](https://leetcode.cn/u/jyd/)
...
* [14.1 剪绳子](#141-剪绳子)：贪心(优先3，余1凑双2 <注意pow()强转为int型>); 动态规划<注意转移方程需要与本体dp[i]比较>; 查表法
* [14.2 剪绳子进阶](#142-剪绳子进阶)：大数不能用dp，贪心用遍历，注意取余时机
* [15. 二进制中 1 的个数](#15-二进制中-1-的个数)：位运算小技巧：(n-1)&n会把最后一位1变为0 <背!!!>
* [16. 数值的整数次方](#16-数值的整数次方)：快速幂(二分)
* [17. 打印从 1 到最大的 n 位数](#17-打印从-1-到最大的-n-位数): 非大数(简单循环)；大数(复杂) <又懵了，先跳过!!!>
* [18.1 在 O(1) 时间内删除链表节点](#181-在-o1-时间内删除链表节点): 双指针
* [18.2 删除链表中重复的结点](#182-删除链表中重复的结点)：力扣没有这题，先跳过
* [19. 正则表达式匹配](#19-正则表达式匹配)：《hard》
* [20. 表示数值的字符串](#20-表示数值的字符串)：<面向测试用例编程跳过>
* [21. 调整数组顺序使奇数位于偶数前面](#21-调整数组顺序使奇数位于偶数前面)：双指针
* [22. 链表中倒数第 K 个结点](#22-链表中倒数第-k-个结点)：快慢指针 & 鲁棒性(特殊值判断)
* [23. 链表中环的入口结点](#23-链表中环的入口结点)：力扣没有这题，双指针+三步()
* [24. 反转链表](#24-反转链表)：迭代(双指针), 递归(相当于遍历链表)
* [25. 合并两个排序的链表](#25-合并两个排序的链表): 递归(判空+判大小); 迭代(伪头结点+判空+返回prehead.next)
* [26. 树的子结构](#26-树的子结构)：递归(先序遍历 + 包含判断)
* [27. 二叉树的镜像](#27-二叉树的镜像): 递归；循环(辅助栈)
* [28. 对称的二叉树](#28-对称的二叉树)：递归(镜像/非镜像--殊途同归)
* [29. 顺时针打印矩阵](#29-顺时针打印矩阵)：模拟打印顺序(原书是找规律)
* [30. 包含 min 函数的栈](#30-包含-min-函数的栈)：辅助栈
* [31. 栈的压入、弹出序列](#31-栈的压入弹出序列): 辅助栈模拟<代码结构：内外循环+辅助栈是否为空判断>
* [32.1 从上往下打印二叉树](#321-从上往下打印二叉树)：BFS <遍历结构：先存root入queue，后循环判断queue是否为空>
* [32.2 把二叉树打印成多行](#322-把二叉树打印成多行)：BFS+双层循环 <遍历结构：内外循环，内循环定义list暂存本层结果，并用size设置内循环次数，内循环i-->
* [32.3 按之字形顺序打印二叉树](#323-按之字形顺序打印二叉树)：BFS+双端队列 <判奇偶层：用标记flag手动交替，或用`res.size()%2`>
* [33. 二叉搜索树的后序遍历序列](#33-二叉搜索树的后序遍历序列)：递归分治
* [34. 二叉树中和为某一值的路径](#34-二叉树中和为某一值的路径)：回溯-状态重置；原书用栈
* [35. 复杂链表的复制](#35-复杂链表的复制)：哈希表；拼接拆分
* [36. 二叉搜索树与双向链表](#36-二叉搜索树与双向链表)：递归-中序遍历 《略迷糊。。》
* [37. 序列化二叉树](#37-序列化二叉树)：《hard》
* [38. 字符串的排列](#38-字符串的排列)：回溯《略迷糊。。》
* [39. 数组中出现次数超过一半的数字](#39-数组中出现次数超过一半的数字): 暴力法；哈希表法；排序；摩尔投票法
* [40. 最小的 K 个数](#40-最小的-k-个数)：库函数排序；基于快排思想的数组划分
* [41.1 数据流中的中位数](#411-数据流中的中位数)：《hard》
* [41.2 字符流中第一个不重复的字符](#412-字符流中第一个不重复的字符)：力扣没这题。。
* [42. 连续子数组的最大和](#42-连续子数组的最大和)：暴力法；贪心；动态规划；分治。
* [43. 从 1 到 n 整数中 1 出现的次数](#43-从-1-到-n-整数中-1-出现的次数)：《hard》
* [44. 数字序列中的某一位数字](#44-数字序列中的某一位数字)：迭代+求整/取余(数学题)
* [45. 把数组排成最小的数](#45-把数组排成最小的数)：大数问题，字符串拼接，自定义排序(模拟快排，同40题)
* [46. 把数字翻译成字符串](#46-把数字翻译成字符串)：递归；dp (类似跳台阶)
* [47. 礼物的最大价值](#47-礼物的最大价值)：dp
* [48. 最长不含重复字符的子字符串](#48-最长不含重复字符的子字符串): 滑动窗口 (dp也可)
* [49. 丑数](#49-丑数): 三指针
* [50. 第一个只出现一次的字符位置](#50-第一个只出现一次的字符位置)：哈希表法/数组替代哈希表法
* [51. 数组中的逆序对](#51-数组中的逆序对)：归并排序 《hard》（i,j反向遍历not AC, 不明原因）
* [52. 两个链表的第一个公共结点](#52-两个链表的第一个公共结点)：双指针a+c+b=b+c+a (不是判断环，不一定循环)
* [53.1 数字在排序数组中出现的次数](#531-数字在排序数组中出现的次数)：二分<排序二分敏感>
* [53.2 0到n-1中缺失的数字](#532-0到n-1中缺失的数字)：二分<抽象目标：求第一个元素与索引值不相等的目标>
* [54. 二叉查找树的第 K 个结点](#54-二叉查找树的第-k-个结点)：二叉查找树中序得升序排列 + 提前终止
* [55.1 二叉树的深度](#551-二叉树的深度)：递归<要看出深度=1+max(左子, 右子)的递归关系>；层序遍历
* [55.2 平衡二叉树](#552-平衡二叉树): 递归-自顶向下(有重复计算的部分); 自底向上(后序遍历，不符的及时枝剪)
* [56.1 数组中只出现一次的数字](#561-数组中只出现一次的数字)：位运算：异或-分组-分别异或 (异或性质：a^a=0)
* [56.2 数组中只出现一次的数字II](#562-数组中只出现一次的数字ii)：位运算：各位求和 对3取余<很迷的位运算 背!!> 哈希表法有何不可
* [57.1 和为 S 的两个数字](#571-和为-s-的两个数字)：暴力法超时；双指针夹逼-二分思想
* [57.2 和为 S 的连续正数序列](#572-和为-s-的连续正数序列)：滑动窗口(双指针遍历)
* [58.1 翻转单词顺序列](#581-翻转单词顺序列): 库函数(trim+split)；双指针一次翻转(没用split)；两次翻转(整体一次，单个单词一次)
* [58.2 左旋转字符串](#582-左旋转字符串)：库函数；列表遍历；三次翻转<其中三次翻转算是比较算法风的>
* [59.1 滑动窗口的最大值](#591-滑动窗口的最大值)：暴力法；双端队列
* [59.2 队列的最大值](#592-队列的最大值)：双端队列<max思路借鉴上题的滑动窗口>
* [60. n 个骰子的点数](#60-n-个骰子的点数)：dp <状态转移方程不好想>
* [61. 扑克牌顺子](#61-扑克牌顺子): 排序+判重+求差 《归纳：顺子max-joker<5时成立》
* [62. 圆圈中最后剩下的数字](#62-圆圈中最后剩下的数字)：数学公式 <记下来>
* [63. 股票的最大利润](#63-股票的最大利润)：暴力法；一次遍历(遍历的是下一个元素对比现有最值并更新)；dp
* [64. 求 1+2+3+...+n](#64-求-123n)：逻辑短路运算符代替if判断
* [65. 不用加减乘除做加法](#65-不用加减乘除做加法)：模拟二进制加法三步走(非进位和/进位/求和)+位运算(异或/位与)+求和循环(while)
* [66. 构建乘积数组](#66-构建乘积数组): 找规律：维护两个三角的数组；改进：右侧的下三角直接累乘到结果数组，节省一个数组的空间
* [67. 把字符串转换成整数](#67-把字符串转换成整数)：分4部分处理字符串，考虑各种特殊值(正数负数, max, min, 空串, null, 非数字字符，混合，越界...)
* [68.1 二叉搜索树的最近公共祖先](#681-二叉搜索树的最近公共祖先)：二叉搜索树；迭代/递归
* [68.2 二叉树的最近公共祖先](#682-二叉树的最近公共祖先)：递归，DFS
<!-- GFM-TOC -->
  ...
  - ### 动态规划：
    ...
    - 思路：
      ...
      - 从而有状态转移方程dp[i] = max(dp[i], max(j * (i - j), j * dp[i - j])) 
        - 注意：还要与dp[i]比较，可能是惯例吧，类比求最大值也是需要与本身比较   
    - 注意：
      - i/2的略微优化没成，就暂时先这样吧..
...
- ## 解题思路
  ...
  - 分析：
    ...
    - 思路：
      - 遍历：每次减去3，最终的乘积最大。长度小于等于4时，不再裁减，保持原长度乘积最大。
    - 每步取模
  - 算法：
    - 初始化2和3等特殊值；
    - 初始化long res = 1；
      - 注意这里定义为long，因为可能超出int范围 
    - while循环：当n>4
      - 更新res，并取模；
      - 更新n；
      - 当n=4或3或2时，退出循环，并在res基础上 乘 剩余的这点n，作为最终res结果
    - 返回res：与res与最终的n相乘，然后取模，然后强转为int。
      - 注意 int 后的内容需要加小括号，否则会测试用例(比如 127)不通过

- ## 代码链接
  - [剪绳子进阶版](https://github.com/anliux/PracticePool/blob/master/jzoffer/src/14_2_剪绳子进阶.java)
...
- ## 解题思路
  ...
  - 不考虑大数问题：返回int[]数组。
    - 直接遍历：
      - n为1时：遍历1到9；10的1次方为10，10-1是9
      - n为2时：遍历1到99；10的2次方为100，100-1=99
      - ...
    - 定义数组来存储这些值：
      - 10的n次方 - 1 得到最大值 
      - 注意：pow()函数的返回值是double型
    - 遍历并将这些数存入结果数组：
      - 数组索引从0开始，而结果从1开始，因此可以赋值时+1 
    - 返回结果数组
  - 考虑大数问题：依次打印符合的值。
    ...
    - 如果累加，计算量大，效率不高，代码冗长；考虑把问题转换成数字排列，每一位从0-9排列一遍，即可得到结果。
      ...
      - 注：原书给出了以上两种方法的求解 
    - 代码终止条件为：
      - 当位数增加到等于数组长度时，相当于已经达到最大值，可以停止了。
    - 使用回溯法得到所有的数。
      - 注意：回溯法一定要注意设置好终止条件。
      - 因为有回溯，所以另外定义函数，方便调用。
...
- ## 解题思路
  - 时间复杂度n：双指针
    ...
    - 算法：
      ...
      - 初始化双指针： pre = head , cur = head.next 。
      - 遍历链表并判断： 当 cur 为空 或 cur 节点值等于 val 时跳出。
        - 保存当前节点索引，即 pre = cur 。
        - 遍历下一节点，即 cur = cur.next 。
      - 删除节点： 当找到指定结点时，跳出循环，执行 pre.next = cur.next 。
      - 返回值： 返回链表头部节点 head 即可。
    - 注：也有单指针或者类似的优化，不过越直白越好，还是用上述方法。
    - PS: 也可以用递归求解 
  - 时间复杂度1：
...
- ## 解题思路
  ...
  - 算法：大循环里套两个小循环和一个交换
    ...
    - 循环交换： 当 i=j 时跳出；
      - i<j 且 指针 i 遇到奇数则执行 i=i+1 跳过，直到找到偶数；
      - i<j 且 指针 j 遇到偶数则执行 j=j−1 跳过，直到找到奇数；
      - 交换 nums[i] 和 nums[j] 值；
      - 此时注意：奇数在前，偶数在后，因此符合这个顺序的时候指针继续走，注意区分i和j应该的奇数偶数的判断
    - 返回值： 返回已修改的 nums 数组。
    - 注：
...
- ## 解题思路
  ...
  - 分析：
    ...
    - 逆推：两个指针，快指针指向尾结点后的null时，慢指针指向所求，则两指针间隔为k不变
      - 原书中是指向尾结点，则间隔为k-1；这里是指向尾结点后的null，则不需要减一，为k 
...
- ## 解题思路
  - 判断链表是否有环：快慢指针
    - 一快一慢：可一个是一步，一个是两步
    - 有环：快追上了慢，即多走了一圈
    - 无环：快指针指向了尾结点的null，仍然没有相遇的话
  - 环的入口结点：快慢指针
    - 需要已知环中的结点数目k
    - 快指针先走k步，然后快慢指针一起
    - 当快慢指针相遇时：相遇的结点即为入口
      - 人为制造了距离k，使得快慢指针在环中形成一个相遇的闭环
  - 环中的结点数：数数
    - 快慢两指针相遇的结点一定在环中
    - 可以从相遇的结点开始计数，当再次回到此结点时，可计数得环中结点数
...
- ## 解题思路
  - 思路：迭代；递归
    ...
    - 迭代重点掌握，递归核心就是遍历链表后慢慢转头。
  - 迭代：
    - 复杂度：时间O(n)，空间O(1)
    - 分析：
      - 遍历链表：用双指针，实现每两个链表结点之间反转，以达到整体反转链表的目的。
      - 其实用到三个指针，因为在两个结点反转的时候，需要把cur存起来，防止丢失next信息使得链表断开。
    - 算法：
      - 定义双指针：当前指针cur指向当前结点，前指针pre指向当前结点的前一个结点；
        - 初始化：cur = head, pre = null (null是因为结果中所有结点的尾结点之后还是null) 
      - 循环：当cur不指向null时遍历链表
  ...
  - 递归：
    ...
    - 递归核心思想：
      ...
      - recur()函数：从recur(1,null) - recur(2,1) -- recur(3,2) -- .. -- recur(5,4) -- recur(null,5), 之后开始反转并回溯 
        - 注：每次返回时，都要反转这两个结点的指向，并返回后一个节点pre，即新的头结点 
    - 算法：
      - recur(cur, pre) 递归函数：
...
- ## 解题思路
  ...
  - 递归：
    ...
    - 比较：将两个链表中头部较小的一个，与剩下元素的 merge 操作结果合并，最终返回此链表
      ...
      - 这里递归调用函数，赋值给小结点的next，并返回小结点，保持链表的链接
        - 注意：传入新的函数后，记得return 取到的小结点
    - 递归的复杂度分析：
      - 时间复杂度：O(m+n)
      - 空间复杂度：O(m+n)
  - 迭代
    - 顺序迭代的思路：
      - 注意：可以设一个伪头结点prehead，方便操作，最后返回prehead.next即可 
    ...
    - 最后加一步对于空链表的分析，并返回prehead
      - 注意：这里是对prev操作，不是对伪头结点，考虑没有跳过的情况，将会只剩下上述循环没有走完的一部分结点 
    - 迭代的复杂度分析：
      - 时间复杂度：O(m+n)
      - 空间复杂度：O(1)
  - 链表相关使用：
...
- ## 解题思路
  ...
  - 算法思路：
    ...
    - isSubStructure(A, B) 函数：负责遍历树A
      - 特例处理： 当 树 A 为空 或 树 B 为空 时，直接返回 false ；
        - 注：题目中指明了 "约定空树不是任意一个树的子结构"，因此AB都是空树也符合false
      - 返回值： 
...
- ## 解题思路
  ...
  - 递归：
    - 复杂度：时间O(n), 空间O(n)
    - 分析：
      - 树的问题一般可以用递归轻松解决，关键是想清楚递归的过程，并注意细节。 
      - 先序遍历，并交换每个节点的左右子树，交换时递归；当root为null时，结束遍历并返回
    - 算法：
      - 判空：root为null时，返回null/或者返回root也一样；
      - 交换root的左右子树，并递归；
        - 注1：交换和递归可以分开，也可以合起来，分开多定义一个变量，但会更对称；
  ...
  - 循环实现：
    ...
    - 分析：
      ...
      - 关键：循环中的当前结点node一直在变化，即循环开始的弹栈结点，并在随后将此结点的子结点压栈，。。，直到null结束。
    - 算法：
      - 判空：root为空时，返回null
      - 初始化栈，并将root根节点添加到栈；
      - 循环：当栈不为空时，循环
...
- ## 解题思路
  - 思路：递归(镜像/非镜像)；迭代
    - 递归的镜像与非镜像其实是差不多的，只是思考的方式稍有差别，不过镜像有上一题的基础，更好理解
  - 镜像-递归
    - 复杂度：时间O(n)，空间O(n)
    ...
    - 定义镜像函数：
      ...
      - 判断顺序：全空true；不空且符合true；否则false。
      - 注：
        - 借鉴100题的递归优化方法先判全空，后判根结点值相等，最后false这种设置可能用空指针异常，判相等一定记得 && 全不空
    - 算法：
      - 主体函数：判空；调用镜像函数。
      - 镜像函数：参数(root,root)
        - 判空：全为空时，返回true；
...
- ## 解题思路
  ...
  - 分析：
    ...
    - 因此，考虑设定矩阵的“左、上、右、下”四个边界，模拟以上矩阵遍历顺序。
      - 注意：top在上但是小的，bottom在下但是大的，不要搞混了 
      - <img width="680" alt="image" src="https://user-images.githubusercontent.com/32152091/182324286-be3821e1-9648-420d-911b-1da587134c15.png">

  - 算法：
    - 复杂度：时间O(mn), 空间O(1) -- m、n为矩阵的行列数。
    - 步骤：判空；初始化边界值和结果数组当前指针；初始化结果数组；循环；返回结果数组。
    - 判空：当数组长度为0时，return new int[0];；
    - 初始化四个边界，以及结果数组的当前索引值；
    - 初始化结果数组res；
    - 循环：死循环，跳出控制在循环体内。
      - 循环体：走一条边 -- 边界值缩小 --  判断是否越界 -- 如果没有，继续下一轮循环
      - 1，按照“从左向右、从上向下、从右向左、从下向上” 的顺序遍历，同时更新各个指标的值。
        - 用for循环完成一条边的遍历：注意循环起始位置是某一边界值；
      - 2，每次遍历完一条边之后，更新边界值，然后进行判断，当边界交错时，break跳出循环。
        - 交错：不包括等于  
    - 返回结果数组res。
  - 注意：
...
- ## 解题思路
  - 思考：
    - 本题的问题所在：弹出最小值以后，需要知道次小值，即需要保存最小值的序列
    - 可以借用辅助栈 
  - 思路：两个栈；一个栈；不借用栈
  - 两个栈：
...
- ## 代码链接
  - [栈的压入、弹出序列](https://github.com/anliux/PracticePool/blob/master/jzoffer/src/31_%E6%A0%88%E7%9A%84%E5%8E%8B%E5%85%A5%E5%BC%B9%E5%87%BA%E5%BA%8F%E5%88%97.java)
...
- ## 解题思路
  - 分析：
    ...
    - 用循环遍历，直到根结点为空。
    - 注：同《27. 二叉树的镜像》中循环的方法，将结点存到某个容器中，并根据遍历的顺序(深度or广度)选择先进先出or先进后出的容器。
      - 27题是DFS，用栈；本题是BFS，用队列(队列可用)
  - 算法：
    - 步骤：判空；初始化；遍历；返回结果。
    - 复杂度：时间O(n)，空间O(n)
    - 判空：当根结点为空时，返回空列表[]（注：根据返回值类型设置）
    - 初始化：
      - 结果列表list：用ArrayList<>()实现。
      - 辅助队列queue：用LinkedList<>()实现Queue结构，并根据循环跳出条件，将根结点存入辅助队列；
    - 循环：当队列 queue 为空时跳出
      - 注：因为循环跳出的条件设置，需要先将root结点存入queue中 
      - 出队：队首元素出队，记为 node；
      - 打印：将 node.val 添加至列表 tmp 尾部；
      - 添加子节点：若 node 的左（右）子节点不为空，则将左（右）子节点加入队列 queue ；
    - 返回值：新建结果数组res，遍历list并存入res，然后返回。
      - List接口的get(index)方法获取指定索引值的元素 
  - 知识点：
    - LinkedList: 
      - 是接口 Queue 的一个实现类
      - List接口的方法：get(int index): 返回此列表中指定位置处的元素。ArrayList也适用。
...
- ## 解题思路
  - 分析：
    ...
    - 关键点：
      ...
      - 设置temp list保存每一层的结果
        - 循环内设置list保存当前层的结果，并设置内循环，以当前层size作为循环次数，倒着遍历，防止size改变造成的影响。
    - 需要用到循环嵌套，但总的循环次数仍是n，因此时间复杂度仍为O(n)。
  - 算法：
    - 步骤：初始化；判空；循环；返回结果。
      - 注意：先初始化后判空，因为根据方法的返回值类型，如果为空，也需要创建list。 
    - 复杂度：时间O(n)，空间O(n)
    - 初始化： 
      - 根据方法的返回值类型设置结果列表，并new ArrayList<>();
      - 定义辅助队列，并用LinkedList<>()使用Queue队列，同时配合循环跳出条件，先添加root结点。
   - 判空：间接判空
     - 配合辅助队列需要在循环前传入root根结点，可以转化为判非空，即非空时才传入结点。
     - 注：根结点判空需要新建，因此可以转为先初始化，然后对根结点判非空   
    - BFS 循环： 当队列 queue 为空时跳出；
      - 新建一个临时列表 tmp ，用于存储当前层打印结果；
      - 当前层打印循环： 循环次数为当前层节点数（即队列 queue 长度），并i--；
        - 出队： 队首元素出队，记为 node；
        - 打印： 将 node.val 添加至 tmp 尾部； -- 注意添加的是node对应的值，不是node本身。
        - 添加子节点： 若 node 的左（右）子节点不为空，则将左（右）子节点加入队列 queue ；
      - 将当前层结果 tmp 添加入 res 。
    - 返回值： 返回打印结果列表 res 即可。
    - 注意：内循环次数控制的for循环
...
- ## 解题思路
  ...
  - 方法一：层序遍历 + 双端队列
    ...
    - 分析：
      ...
      - 核心思路：
        - 本方法主要在写结果的顺序做文章。而下面的方法还需要考虑存入辅助队列时的顺序。
      - 注：建立在上一题的方法基础上。
    - 算法流程：
    ...
    - PS: 
      - 巧妙，但是也绕，更亲民的方法是定义flag
      - flag：每次进循环，先判断flag，后进行奇数层或偶数层操作，然后更新flag，和下面的方法有一点相似。
      - flag设置：`flag=true; flag=!flag;`或`flag=1; flag^=1;`    
      - 注意：flag的方法和方法2更类似，还需要考虑存入辅助队列的顺序，也很绕，千层饼。                                                          
  - 方法二：层序遍历 + 双端队列 + 奇偶分离
    - 复杂度：时间O(n), 空间O(n)
    - 分析：
    ...
    - 本方法关键点：
      - 将奇 / 偶数层作为一组，都写出来，并在中间进行一次判断是否有偶数层
    - 算法流程：
      - 与方法一对比，仅 BFS 循环不同。
    ...
    - 注：
      - 用到双端队列deque，及其方法addFirst(), addLast() 和 removeFirst(), removeLast()
  - 综上：
    - 以当前水平，建议用法1的flag，或法2.       
...
- ## 解题思路
  ...
  - 算法:
    ...
    - 递推工作：
      - 划分左右子树： 
        - 遍历后序遍历的 [i, j] 区间元素，寻找 第一个大于根节点 的节点，索引记为 mid 。
        - 此时，可划分出左子树区间 [i,mid-1] 、右子树区间 [mid, j - 1] 、根节点索引 j 。
      - 判断是否为二叉搜索树：
        - 左子树区间 [i, mid - 1] 内的所有节点都应 < postorder[j] 。而《第 1.划分左右子树》 步骤已经保证左子树区间的正确性，因此只需要判断右子树区间即可。
        - 右子树区间 [mid, j-1] 内的所有节点都应 > postorder[j] 。实现方式为遍历，当遇到 ≤postorder[j] 的节点则跳出；则可通过 p = j 判断是否为二叉搜索树。
    - 返回值： 所有子树都需正确才可判定正确，因此使用 与逻辑符 双与 && 连接。
      - p = j ： 判断 此树 是否正确。
      - recur(i, mid - 1) ： 判断 此树的左子树 是否正确。
      - recur(mid, j - 1) ： 判断 此树的右子树 是否正确。 

- ## 代码链接
  - [二叉搜索树的后序遍历序列](https://github.com/anliux/PracticePool/blob/master/jzoffer/src/33_%E4%BA%8C%E5%8F%89%E6%90%9C%E7%B4%A2%E6%A0%91%E7%9A%84%E5%90%8E%E5%BA%8F%E9%81%8D%E5%8E%86%E5%BA%8F%E5%88%97.java)
...
- ## 代码链接
  - [二叉树中和为某一值的路径](https://github.com/anliux/PracticePool/blob/master/jzoffer/src/34_%E4%BA%8C%E5%8F%89%E6%A0%91%E4%B8%AD%E5%92%8C%E4%B8%BA%E6%9F%90%E4%B8%80%E5%80%BC%E7%9A%84%E8%B7%AF%E5%BE%84.java)
...
- ## 题目描述
  ...
  - <img width="680" alt="image" src="https://user-images.githubusercontent.com/32152091/183336441-facaef00-cd15-4867-bb97-b4661a5e87a8.png">
  - <img width="680" alt="image" src="https://user-images.githubusercontent.com/32152091/183336485-c69593d3-5a0e-4d0f-9474-01e05270459a.png">
  - 提示：
...
- ## 题目描述
  ...
  - 注意：原书是双向有序链表，本题加了“循环”。

- ## 解题思路
  - 分析：
    - 二叉搜索树的中序遍历为 递增序列 ； 二叉树递归实现。
...
- ## 代码链接
  - [二叉搜索树与双向链表](https://github.com/anliux/PracticePool/blob/master/jzoffer/src/36_%E4%BA%8C%E5%8F%89%E6%90%9C%E7%B4%A2%E6%A0%91%E4%B8%8E%E5%8F%8C%E5%90%91%E9%93%BE%E8%A1%A8.java)
...
- ## 解题思路
  - 核心思路：回溯
    - 分解问题：每一位逐层深入，遍历并与后面字符交换；之后固定已遍历位，继续深入下一位 
    - 排列字符数组，并将字符数组每次以字符串的形式返回保存
  - 排列方案的生成：
  ...
  - 算法流程：
    - 终止条件：
      - 当 x = len(c) - 1 时，代表所有位已固定（最后一位只有 1 种情况），则将当前组合 c 转化为字符串并加入 res ，并返回；
    - 递推参数： 
      - 当前固定位 x ；
  ...
  - 知识点：
    - String 的静态方法：`static String 	valueOf(char[] data) `
      - 返回 char 数组参数的字符串表示形式。字符数组的内容已被复制，后续修改不会影响新创建的字符串。
      - 参数：data - char 数组。
      - 返回：一个新分配的字符串，它表示包含在字符数组参数中的相同字符序列。
    - LinkedList的转为数组的方法`toArray(T[] a)`: 
      - 返回数组，返回数组的运行时类型为指定数组的类型 
      - 假定 x 是只包含字符串的一个已知列表。以下代码可用来将该列表转储到一个新分配的 String 数组：
        - 示例：`String[] y = x.toArray(new String[0]); `

- ## 代码链接
  - [字符串的排列](https://github.com/anliux/PracticePool/blob/master/jzoffer/src/38_%E5%AD%97%E7%AC%A6%E4%B8%B2%E7%9A%84%E6%8E%92%E5%88%97.java)
...
- ## 解题思路
  ...
  - 暴力法：
    ...
    - 算法：初始化n/2；双层循环：外层控制比较基准，内层统计与基准相同的个数，并在内层结束后与n/2比较；return任一int型，比如-1.
    - 不足：时间复杂度过高
  - 哈希表：
    - 复杂度：时间O(n)，空间O(n)
    ...
    - HashMap知识点：
      ...
      - `containsKey()`：需要指出查找的是key还是value
  - 排序：
    - 复杂度：时间O(nlogn)，空间O(1)
  ...
  - 摩尔投票法：
    ...
    - 算法：
      ...
      - 遍历数组：与基准相同+1，不同-1，且变为0时，更新基准为下一个索引对应的元素；
        - 循环体中，先判决count是否为0，后判nums[j]是否等于flag，一前一后用if-else连接；
        - 否则，赋值flag和计数器count会冲突，每次更新一次count即可。
        - else里嵌套判断的情况：可以用三元运算符代替      
      - 返回最后计数器不为0的基准。
...
- ## 解题思路
  - 思路：
    - 库函数排序；基于快速排序的数组划分。
  - 分析：
    - 排序：最基本的思路，先排序后取前k小的数字。复杂度O(nlogn).
    - 快速排序的数组划分：二分思想 + 快排基准数两侧不一定是排好序的
  - 库函数排序：
    - 对原数组从小到大排序后取出前 k 个元素
    - 排序函数：`Arrays.sort(数组名);`
    - 取前k个元素的数组：
      - 可以新建后遍历前k个元素并存入结果数组；
      - 可以用静态方法：`Arrays.copyOf(数组名, 截取的数组长度)`
  - 快速排序
    - 快排的两个核心思想：哨兵划分和递归
    - 哨兵划分操作： 
    ...
    - 步骤：
      - 递归出口：子数组长度为 1 时终止递归;
      - 初始化哨兵指针为left和right；
      - 循环：当哨兵指针相遇时终止(基准可以后动，评论看到有人反了报错）
        - 循环：从右向左找第一个小于基准数的元素；
        - 循环：从左向右找第一个大于基准数的元素；
        - 交换两个哨兵元素；
...
- ## 代码链接
  - [最小的 K 个数](https://github.com/anliux/PracticePool/blob/master/jzoffer/src/40_%E6%9C%80%E5%B0%8F%E7%9A%84k%E4%B8%AA%E6%95%B0.java)
...
- ## 代码链接
  - [数字序列中的某一位数字](https://github.com/anliux/PracticePool/blob/master/jzoffer/src/44_%E6%95%B0%E5%AD%97%E5%BA%8F%E5%88%97%E4%B8%AD%E6%9F%90%E4%B8%80%E4%BD%8D%E7%9A%84%E6%95%B0%E5%AD%97.java) 
...
- ## 解题思路
  - 思路：自定义排序
    - int拼接需要考虑大数问题，而大数问题可以转为字符串解决。
    - 字符串拼接："+"拼接
    - 自定义排序：
      - `x+y<y+x`: 则x在前，y在后，类似x"小于"y
      - 这种前后顺序，类似数字排序后的"小在前大在后"的顺序，因此可以定义一种新的“排序”
  - 自定义排序：
  ...
  - 算法：
    - 初始化： 字符串列表 strs ，保存各数字的字符串格式；
      - 首先：定义字符串数组；
      - 其次：将数字作为字符串元素保存到字符串数组中。
    - 列表排序： 应用以上 “排序判断规则” ，对 strs 执行排序；
      - 自定义：模拟快排；<参考40. 最小的k个数>
      - 内置函数：java用java8新特性lambda表达式 `->`，不太熟悉，建议用模拟快排
    - 返回结果： 拼接 strs 中的所有字符串并返回
      - 定义可变字符串 StringBuilder sb
      - 遍历并拼接到sb中
      - 注：此时strs中的字符串为已经按照顺序排好的。
  - 复杂度：
    - 时间O(nlogn), 空间O(n) 
  - 知识点:
    - String的方法：
...
- ## 代码链接
  - [把数组排成最小的数](https://github.com/anliux/PracticePool/blob/master/jzoffer/src/45_%E6%8A%8A%E6%95%B0%E7%BB%84%E6%8E%92%E6%88%90%E6%9C%80%E5%B0%8F%E7%9A%84%E6%95%B0.java)
...
- ## 解题思路
  - 分析：
    - 思路同跳台阶，可用递归或者dp；
    - 不同的是：可跳的台阶有了限制条件，即不符合翻译条件的数字不可用。 
    - 转移方程： 
      - 若 x_i 和 x_{i-1}  组成的两位数字可以被翻译，则 dp[i] = dp[i - 1] + dp[i - 2] ；
      - 否则 dp[i] = dp[i - 1]dp[i]=dp[i−1] 。
      - 可被翻译的两位数区间：当 x_{i-1} = 0 时，组成的两位数是无法被翻译的（例如 00, 01, 02, ..），因此区间为 [10, 25] 。
  - 图示：
    - <img width="549" alt="image" src="https://user-images.githubusercontent.com/32152091/184829455-57086c65-c2d3-459f-b0eb-38871a235453.png"> 
  - 算法：
    - 递归：更好理解
    - dp：在递归基础上理解
      - 设第i位为x_i, dp[i] 代表以 x_i 为结尾的数字的翻译方案数量
        - 显然dp[1]=1, 而符合的dp[2]=2，倒退dp[0]=1
      - dp[i] = dp[i-1] + dp[i-2], 当后两位可以翻译，即属于[10,25]时
      - dp[i] = dp[i-1]，当后两位不能翻译时
      - 注：此题的从左往右和从右往左遍历是一样的效果，因此如果转为字符串遍历，可以按从0到最后遍历

- ## 代码链接
  - [把数字翻译成字符串](https://github.com/anliux/PracticePool/blob/master/jzoffer/src/46_%E6%8A%8A%E6%95%B0%E5%AD%97%E7%BF%BB%E8%AF%91%E6%88%90%E5%AD%97%E7%AC%A6%E4%B8%B2.java)
...
- ## 解题思路
  ...
  - 算法：
    ...
    - 内外循环求剩余dp[]的值；
    - 返回dp数组最右下角的值。
  - 改进：dp数组加一行一列，不用单独考虑边界
    - 第一行第一列默认值为0，直接从i=1，j=1开始计算，避开了单独求第一行和第一列的边界值  
    - 注意：i-j循环对应的是`gird[i-1][j-1]`, 否则会越界。
    - 其他dp问题也可以做类似的操作，多扩一个空间有时候很多边界条件会更简单。

- ## 代码链接
  - [礼物的最大价值](https://github.com/anliux/PracticePool/blob/master/jzoffer/src/47_%E7%A4%BC%E7%89%A9%E7%9A%84%E6%9C%80%E5%A4%A7%E4%BB%B7%E5%80%BC.java)
...
- ## 解题思路
  ...
  - 面试思路：三指针法
    - 分析：思路大致同改进思路。
      ...
      - 求得最小值对应的指针，并更新。
    - 算法：
      - 初始化；for遍历直到n：取min，判断并更新相应的指针；返回结果
...
- ## 解题思路
  ...
  - 哈希表：
    ...
    - 时间复杂度：O(N)，空间复杂度：O(1)
    - 算法：
      - 步骤：初始化；第一次遍历：存表；第二次遍历：查找；找不到时返回单空格。
      - 定义哈希表/数组；
...
- ## 解题思路
  - 暴力法：
    - 遍历整个数组，并逐个比较当前元素与后面元素大小。
    - 时间复杂度： O(N^2)
    - 相对较慢，力扣运行会越界
  - 归并排序：归并排序与逆序对是息息相关的
    - 归并排序体现了 “分而治之” 的算法思想
    - 分：
      - 不断将数组从中点位置划分开（即**二分法**），将整个数组的排序问题**转化为子数组**的排序问题；
    - 治：
      - 划分到子数组长度为 1 时，开始向上合并
      - 不断将 **较短排序数组** 合并为 **较长排序数组**，直至合并至原数组时完成排序；
    - 时间复杂度：O(NlogN)，空间复杂度： O(N) -- 用空间换时间
    - 归并排序与逆序对：
      - **合并阶段** 本质上是 **合并两个排序数组** 的过程
      - 每当遇到 左子数组当前元素 > 右子数组当前元素 时，意味着 「左子数组当前元素 至 末尾元素」 与 「右子数组当前元素」 构成了若干 「逆序对」
      - 注：原书中是倒序遍历的，即为：「左子数组当前元素」 与 「右子数组当前元素及左侧所有剩余元素」 构成了若干 「逆序对」
  - 归并排序计算逆序对算法：
    - merge_sort()函数：
      - 终止条件：当 left≥right
        - 当两端指针碰头时，拆分到了最小格子，子数组长度为 1
        - 单个格子不存在逆序对，返回0
      - 递归划分：
        - 初始化并计算数组中点 mid:
          - left + right, 并除以2 
        - 初始化逆序对res：
          - 递归划分左子数组 merge_sort(l, m) 和右子数组 merge_sort(m + 1, r)，并相加赋值到res
      - **合并与逆序对统计**：分别从两组的左起元素向后遍历，对比并计数 (res只有一种情况时，才计数)
        - **步骤**：
          - 1，定义2指针；
          - 2，[left,right]内已排序顺序暂存到tmp；
          - 3，遍历[left,right]并针对i,j对应元素大小的3种情况分类处理
        - **核心思路**：心中有两组分开数组和一组合并数组，以及对比后将元素存入nums的概念(类似书中图示) 
        - step1: 定义2指针
          - 分别从两组数组的最左侧开始，向后遍历 
        - step2：暂存现有排序顺序到辅助数组
          - 暂存数组 nums 闭区间 [left,right] 内的元素至辅助数组 tmp ；
          - 即遍历，将本组的两组数组的原有顺序(即已经对比排序过的)复制到tmp数组
        - step3: 循环合并，遍历本组从left到right范围内的两组数组，2指针对应元素大小的3种情况进行分类处理
          - 注意：k 对应即将放入 nums 的元素位置，i,j 两指针，每轮分别进行更新
          - 游戏规则：每轮将i，j对应元素中较小的一个放入nums，并指针前移1位
          - 3种情况处理顺序：先两种某一组已对比完的情况，后处理找到逆序对的情况；其中有两种处理相同，可合并
          - 当 i=m+1 时：
            - 即前一组已遍历到头：将后一组元素依次放入nums数组
            - 因此添加右子数组当前元素 tmp[j] ，并执行 j=j+1 ；
          - 否则，当 j=r+1 时：
            - 即后一组已遍历到头：将前一组元素依次放入nums数组
            - 因此添加左子数组当前元素 tmp[i]，并执行 i=i+1 ；
          - 否则，当 `tmp[i]≤tmp[j]` 时：
            - 前<后，即非逆序时，将前一组元素放入nums，并i++，
            - 添加左子数组当前元素 tmp[i] ，并执行 i=i+1，与上一种情况的处理方式相同，可以合并处理
            - 注意：= 的情况，取小值时，优先将小数组(左侧数组)的值放入nums；反之，取大值时，优先将大数组(右侧数组)放入nums
          - 否则（即 `tmp[i]>tmp[j]`）时：
            - 前>后：存在逆序对
            - 添加右子数组当前元素 tmp[j] ，并执行 j=j+1 ；
            - m−i+1 个逆序对添加至 res：i到第二组首位mid+1之间的所有元素，含i，不含mid+1
      - 返回值：
        - 返回直至目前的逆序对总数 res ；
    - reversePairs() 主函数：
      - 初始化：
        ...
        - 注意：tmp和nums声明为全局变量
      - 返回值：
        - 执行归并排序 merge_sort(0, nums.length-1) ，并返回逆序对总数即可；
   - 代码语法：
     - this 关键字：来引用成员变量，以区分局部变量和成员变量
  - 错误代码：
    - 企图每组从右往左指针遍历的方法不明原因报错：[4,5,6,7,8,9]，输出：-3？
    - 应该是合并遍历里的res改动那句有问题，但是逻辑没搞通，先这样吧（错误代码在link 里有备份）

- ## 代码链接
  - [数组中的逆序对](https://github.com/anliux/PracticePool/blob/master/jzoffer/src/51_%E6%95%B0%E7%BB%84%E4%B8%AD%E7%9A%84%E9%80%86%E5%BA%8F%E5%AF%B9.java) 
...
- ## 解题思路
  ...
  - 方法三：双指针法 
    ...
    - 思路：
      - 链表A：a+c, 链表B : b+c. 
      - a+c+b+c = b+c+a+c 。则会在第二个c起点相遇。
      - 若不相交，c=0, 则a+0+b = b+0+a 。因此相遇处是NULL（尾）
    - 算法：
      - 判空；新建结点；循环：两结点移动，直到相等。
    - 注：
      - 指针要另外定义，不能直接用头结点啊！
      - 没有公共时，最后都指向null，此时相等，退出循环。因此不会死循环。
    - 代码使用了三目运算符，很巧妙。
...
- ## 解题思路
  ...
  - 二分法：排序数组的敏感，先右后左。
    ...
    - 算法：
      - 步骤：初始化两指针；第一次遍历，找到右边界；重置两指针；第二次遍历找到左边界；返回差值的结果。
      - 初始化边界指针为数组的左右端点；
      - 两次二分；
        - 不等时，同二分处理；
        - nums[mid]与target相等时：
      ...
      - 注意：
        - 注意循环结束时，取到的值是哪个指针。 
        - target不存在的情况会返回0，符合预期结果。
        - 注意：最后计算结果的是左右边界值，不是两指针!!!
    ...
    - dfs：
      ...
      - 本节点的三项工作：
        - 提前返回： 若 k=0 ，代表已找到目标节点，无需继续遍历，因此直接返回；
          - 加一个判断：为了终止循环，否则返回结果以后，递归中的dfs仍然没有终止。 
...
- ## 解题思路
  - 递归的思想：
    - 考虑“记录二叉树的路径”的方法比较繁琐，规律可知用递归比较快。
    - 可以用三元运算符 A?B:C 将代码化简为一句代码。
    - 关键是：看清”深度=1+max(左子，右子)“的递归关系。
  - 分析：
    - 如果只有根节点：深度为1
    - 如果只有左子树：深度为左子树的深度+1
    - 如果只有右子树：深度为右子树的深度+1
    - 如果既有左子树又有右子树：深度为1 + 左右子树的深度中的较大值
    - 结合图示更清晰
  - 算法：
    - 递归出口：越过叶节点时返回0 (注意需要返回一个数值)
    - 递归：深度 = 1 + max(左子树深度， 右子树深度)
  - BFS:
    - 另外，还有一种层序遍历的思路，每遍历一层，count+1 
...
- ## 解题思路
  - 思路：dfs -- 自顶向下，自底向上 1ms/56.05%; 41.3 MB/20.55%
  ...
  - 自顶向下（先序遍历-暴力法）：
    ...
    - 算法：
      - 判空返回真；
      - 调用height函数判断子树高度差<2, 以及调用本体函数判断左右子树是否平衡
  ...
  - 自底向上（后序遍历-提前枝剪）：
    ...
    - 算法：
      - 使用与方法一中定义的 height 方法。逻辑相反，首先判断子树是否平衡，然后比较子树高度判断父节点是否平衡。
        - 设定：不符合平衡树时返回-1，后面也是判断返回值是否是-1来判断boolean值 
      - 算法如下：
        - 主体函数直接调用递归函数recur()：
          - 判断返回值是否是-1
        - recur()函数： 仍是求depth深度函数
          - 判空：root为null时返回0；
          - 左子树深度：初始化并递归；
          - 判断左子树是否是平衡树：为-1时直接跳出
          - 右子树深度：初始化并递归；
          - 判断右子树是否是平衡树：为-1时跳出
          - 返回本节点是否是平衡树：左右子树的深度abs值是否<=1
  - 区别：
  ...
  - 分析：
    ...
    - 考虑异或：a^a=0的性质
      ...
      - 当有两个数字只出现一次时，遍历并全部异或的结果是所求两个数字异或的结果。
        - 不能直接得到，但是这个结果表明了这两个数的不同位 
        - 找到第一个为1的位即可作为分组依据
      - 根据结果将所有数字分为两组：
        - 结果中二进制位1的位数即为两个数字相异的位数，可以根据这一位进行分组
        - 其他相同元素：任一位都是相同的，因此这样划分不会被分到两个组里
      - 分组之后分别异或，即可得到最终的两个数字。
        - 注：分组不用实际分开，就if-else分流到并异或到不同的结果变量即可。
  - 算法：
    - 步骤：初始化结果变量；全部异或；左移获取最低为1位；结果数组a和b；遍历：if-else分流；返回结果。
...
- ## 解题思路
  ...
  - 分析：
    ...
    - 两种方法：有限变量自动机；遍历统计。
      ...
      - 用哈希表记录次数应该也是可以的，只是空间复杂度O(n)，而位运算空间复杂度为O(1).
  - 算法：
    - 步骤：初始化数组；内外循环：统计每位的1；初始化结果变量和参数3；遍历：每位取余3；返回结果
      - 涉及多次的位运算 
...
- ## 解题思路
  - 思路：暴力法；双指针
    ...
    - 双指针：
      - 顺序数组想二分
      - 本题二分的点是：根据sum与target的大小关系，来移动大小指针，sum大则往小缩，sum小则往大走，两边逼近。
  - 双指针算法：
...
- ## 解题思路
  ...
  - 基本步骤：
    ...
    - 当cur=sum时，循环将窗口内的序列添加到list内，并small++或big++来继续寻找符合的序列。
      - 最好是small++，防止big超出范围
    - 当small追上big时，表示不再有符合条件的序列了。跳出循环。
  - 算法流程：
    - 步骤：初始化：指针和结果列表；循环：根据结果移动两指针；遍历列表存入结果数组；返回结果
    - 初始化： 
      - 左边界 i = 1 ，右边界 j = 2 ，元素和 s = 3 
      - 结果列表 list：根据返回值类型确定，用到ArrayList的add功能，最后注意转换
    - 循环： 当 i≥j 时跳出；
      - 当 s > target 时： 向右移动左边界 i = i + 1 ，并更新元素和 s ；
      - 当 s < target 时： 向右移动右边界 j = j + 1 ，并更新元素和 s ；
      - 当 s == target 时： 记录连续整数序列，更新s，并向右移动左边界 i = i + 1；
        - 新建数组，`j-i+1`求得长度，并通过遍历存入。  
      - 注意：
        - 整个循环中，每次移动指针时都需要更新s的值；
        - 如果是每次循环之前按照等差数列求和公式计算sum，则不用。
    - 返回值： 
      - 遍历列表：将list中的每个元素存入结果变量
      - 返回结果列表 res ；
      - 注：这里可以直接用ArrayList的一个函数：`<T> T[]toArray(T[] a)`，但是不太熟悉，多用多错不如遍历。
  - 注意：
...
- ## 解题思路
  - 思路：
    - 库函数；列表遍历；三次翻转。
  - 库函数：
    - 用`substring(start,end)`函数和"+"直接返回结果。
    - substring(): 注意左闭右开，两个参数的取值   
  - 列表遍历：
    - 借用可变字符串StringBuilder，获取单个字符并将它们按照题目要求的顺序连接   
  - 三次翻转
...
- ## 解题思路
  ...
  - 暴力法：
    - 暴力法的难点：
      - 滑动时，之前窗口的最大值可能会滑出窗口外，因此每次需要遍历窗口内所有元素
    - 复杂度：时间O(nk)，空间O(n-k+1)，n是数组元素个数
      - 一共有 N - k + 1 个滑动窗口，每个有 k 个元素，于是算法的时间复杂度为 O(Nk)，容易超时。
    - 算法：
      - 步骤：判空；定义结果数组；内外循环遍历；返回结果。
      - 判空：`if(nums==null && k==0)` 则新建长度为0的数组并返回
        - 注：测试用例`输入：[] 0; 输出：[]`，因此判空不能返回null，需要返回新建长度为0的数组
      - 定义长度n=nums[].length：
        - 注：为了方便使用，也可以不定义，每次计算。
      - 定义结果数组，数组长度为滑动窗口个数 `n-k+1`
      - 两层循环：外层控制起始，内层控制小窗口内的遍历
        - 定义最大值max=nums[i];
        - 内循环：遍历本窗口内的所有数字并更新max，然后将max作为res[i]存入结果数组
      - 返回结果数组res
  - 双向队列
    - 复杂度：时间O(n)，空间O(n)
    - 存储双向队列的索引，以优化时间复杂度
      - 队首存储每个窗口的最大值，队尾存储可能成为最大值的其他元素（当最大值滑出后）
        - 即最大值添加在队首，滑动后新加入的元素添加在队尾。
        - 非严格递减队列：即队首max，后面添加到队尾的都要比max小，否则更新队列(循环实现弹出所有小于最新元素的值)
        - 具体：max添加到队首，小于max的添加到队尾作为max预备元素；遇新的元素，循环删掉小于新元素的所有元素，并添加在队尾。
      - 先遍历第一个窗口，找出最大值；然后从第二个窗口开始遍历，以每个窗口最右端为基准，遍历至最后一个元素
      - 清理双向队列:
        - 仅包含窗口内的元素
          - 每轮窗口滑动移除了元素 nums[i - 1] ，需将 deque 内的对应元素一起删除。
        - 非严格递减：
          - 每轮窗口滑动添加了元素 nums[j + 1] ，需将 deque内所有 < nums[j + 1] 的元素删除
    - 算法：
      - 步骤：判空；初始化：双端队列和结果数组；构建第一个窗口；窗口滑动：更新队列中的元素；返回结果数组res。
        - 看起来代码更长，但是思路更清晰，合在一起会增加很多冗余判断。 
      - 判空：`if(nums==null && k==0)`；
      - 初始化：双端队列 deque，结果数组 res
        - 新建数组双端队列deq：`ArrayDeque<Integer> deq = new ArrayDeque<Integer>();`
      - 构建窗口：
        - 遍历第一个窗口，找出第一个最大值；
        - 循环 - 判断是否需要更新max：
          - 新加入元素：非空 且 队中有元素小于新元素时，删去小的元素。
          - 添加新元素到队尾。
          - 注意此处是循环不是if判断
        - 添加在队尾
      - 结果res：第一个元素赋值为list.peek的值
      - 窗口滑动：以每个窗口的 最右端 为基准，从第二个窗口开始，遍历至数组末尾最后一个元素
        - 删去滑出元素：
          - 判断：删去新添加的元素：当删去元素是max时，删去队首元素（i-k是刚刚滑出本窗口的索引值）；
        - 更新max：
          - 循环（可能存在多个不符的队尾元素）：当队列不为空，且当前元素大于队尾时，移除队尾元素；
          - 这里：每新加入一个元素，就循环从队尾删掉一些小于新元素的队列中元素
        - 添加新滑进来的元素：
          - 将当前元素添加到队列last（作为在最大值滑出窗口后，可能最大值的预备元素，放入队尾）
        - 获取本组窗口的max，存入结果数组
      - 返回结果数组res
      - 注：
        - peekFirst/peekLast:
          - 与 getFirst/getLast 相似的功能;
          - 区别是如果为空，则返回null；而get会抛出异常NoSuchElementException
          - 因此可以优先使用peek功能
...
- ## 解题思路
  - 思路：双端队列维护max
  - 分析：要求时间复杂度是O(1)
    - 压尾、弹首：符合队列的先进先出，可以实现O(1)
    - max：借鉴上一题的思路，可以用双端队列实现 
  - 算法：
    ...
    - 设计双向队列为 单调不增 的原因：
      - 若队列 queue 中存在两个 值相同的最大元素 ，此时 queue 和 deque 同时弹出一个最大元素，而 queue 中还有一个此最大元素；
      - 即采用单调递减将导致两队列中的元素不一致。

- ## 代码链接
  - [队列的最大值](https://github.com/anliux/PracticePool/blob/master/jzoffer/src/59_2_%E9%98%9F%E5%88%97%E7%9A%84%E6%9C%80%E5%A4%A7%E5%80%BC.java)
...
- ## 解题思路
  - 思路：暴力法；动态规划；查表法
  - 分析：
    - 1个骰子：6个面，结果范围 [1,6]，6-1+1 种结果：每种的概率：1/6;
    - 2个骰子：12个面， 结果范围 [2,12], 12-2+1 种结果 
    - n个骰子，6n个面，结果范围 [n,6n], 6n-n+1 种结果
  - 暴力法：
    - 依次计算每种结果的概率
    - n越大，计算量越大。。
  - 动态规划：
    ...
    - 步骤：
      - 初始化结果数组和辅助数组；辅助数组i=1赋值；遍历求dp；将辅助数组的值转移到结果数组；返回结果数组。
      - 初始化结果数组和辅助数组：
        - 结果数组：一维；辅助数组：二维。
      - 辅助数组i=1赋值：
        - 初始化1个骰子的基本情况的数值
      - 遍历，填充辅助数组dp
        - 第一层for i：从1-n遍历骰子数
        - 第二层for j：从i- 6i遍历i个骰子对应的点数和 `[i,6i]`
        - 第三层for k：从1-6遍历新增骰子可能的点数
      - 将辅助数组的值转移到结果数组:
        - `res[i] = dp[n][n+i]` 
      - 返回结果数组res
  - 查表法：
...
- ## 代码链接
  - [n 个骰子的点数](https://github.com/anliux/PracticePool/blob/master/jzoffer/src/60_n%E4%B8%AA%E9%AA%B0%E5%AD%90%E7%9A%84%E7%82%B9%E6%95%B0.java)
...
- ## 解题思路
  ...
  - 分析：
    - 顺子：
      - 如果有重复，一定不是顺子，因此判重时直接返回false；
      - 如果遇到0，统计起来，0可以插到任意空中。
      - 极限情况：除0外，min需要放在nums[0]的位置，则在0插空的前提下，两个端的值需要差值<5，此时0补充的情况下正好为顺子
      - 其他情况：即差值为3 2 1等，此时0补充在了首尾而不是在中间的情况
...
- ## 解题思路
  ...
  - 分析：
    ...
    - 逐项累加：
      - 用到循环--排除
    - 递归：
      - 不能用循环的情况下，只能用递归来实现循环的过程；
      - 但递归的终止条件需要使用 if
      - 如果不用if来判断递归终止？
    - &&：
      - 短路可以用来做判断，前一个成立则做后一个，前一个不成立时，后一个不再做。
    - 综上：
      - 可以利用短路 && 来实现 if 的功能；利用递归来实现循环while的功能
  - 递归： 
  ...
  - 算法：
    - 定义布尔参数flag构建双与递归:
      - 双与前：判断n>1
      - 双与后：递归，累加sumNum(n-1)到n；注意式子是boolean型，最后加一个大于小于判断
        - 注意：双与连接的都是布尔型变量，可以在进行计算的`n+=..`部分加一个`>0`之类的判断。
    - 返回n; 
    - 注意运算符优先级：
...
- ## 解题思路
  - 分析：
    ...
    - 其他规律：
      - 循环停止的条件是，当两个数中有一个数变为0时，返回另一个数
  - 关键规律：
    - 两个数异或：相当于每一位相加，而不考虑进位；
    - 两个数相与，并左移一位：相当于求得进位；
    - 将上述两步的结果相加，进位不为0则递归
  - 算法：
    - while循环：当 进位carry/b 不为0时
      - 初始化 非进位和sum，并异或a b求值：`int sum = a ^ b;`
      - 初始化 进位carry，并位与a b求值：`int carry = (a & b) << 1;`
      - 将上述两个结果再次赋值给a b做求和运算：`a = sum; b = carry`
    - 返回非进位结果a (进位为0时才退出循环)
    - 注意：使用`do{...}while(..);`格式时，最后的分号不要丢！
...
- ## 解题思路
  - 找规律：维护两个三角的数组；
     - 改进：右侧的下三角直接累乘到结果数组，节省一个数组的空间
  - 分析：
    - 直接法：乘积除以每个a[i]，但用到除法，不符合要求。
    - 连乘法：需要o(n^2)的时间构建整个数组，有待优化。
    - 发现规律：
      - 如果将缺少的连乘元素设为1，就可以得到一个对角线是1的矩阵，同时其他元素被划分为上下两个三角区域。
      - 左侧数组：递增，从上到下构建，规律是left(n)=left(n-1)*a(n-1);
      - 右侧数组：递减，从下到上构建，规律是right(n)=right(n+1)*a(n+1);
      - 构建结果数组：左右两个三角数组的同一行乘积可以得到结果行
      - 图示：
        -  ![图片](https://github.com/anliux/PracticePool/assets/32152091/8882836e-6be5-4d2c-aaee-0559f2ea2487)

  - 算法：
    - 判空：为空或数组长度为0时，返回数组a；
    - 初始化数组长度len
    - 初始化左右两个数组，并分别将首尾的元素赋值
      - `left[0] = right[len-1] = 1;` 
    - 循环构建左侧数组：
      - 递增，从上到下构建，规律是`left(n)=left(n-1)*a(n-1);` 注意起始位i=1
    - 循环构建右侧数组：
      - 递减，从下到上构建，规律是`right(n)=right(n+1)*a(n+1);` 注意起始位i=n-2
    - 循环构建结果数组：
      - 左右两个三角数组的同一行乘积
    - 返回结果数组
  - 改进算法：借助一个辅助变量，直接累乘到先构建的左侧三角数组中
    - 判空
    - 初始化结果数组，并对ans[0]赋值
    - 定义辅助变量tmp，并赋值为1
    - 循环构建左侧三角数组：同上
...
- ## 题目描述
 ...
 - ## 解题思路
  - 多种情况case处理
    - 将字符数字转换为数字的功能本身是比较简单的
    - 复杂的是：需要考虑各种特殊值输入的处理
  - 分析：
    - 一段str可能是由4部分构成的：
      - | 空格 | 符号位 | 字符数字 | 非字符数字 |
      - ![1](https://github.com/anliux/PracticePool/assets/32152091/da365c23-c10a-4f89-a77e-59cf5ddf1981)
    - 空格处理：首部空格删除即可，`str.trim()` 修剪掉前后的空格
    - str转换为字符数组：`str.toCharArray()`
    - 符号位：'+', '−', '无符号', 新建一个变量保存符号位，返回前判断正负即可
      - 注：为负则sign=-1，否则如果不为正，则返回 0
    - 数字字符：需要考虑越界问题处理
      - 只能存储 32 位大小的有符号整数，那么其数值范围为 [−2^31,  2^31 − 1]。如果数值超过这个范围，返回  INT_MAX (2^31 − 1) 或 INT_MIN (−2^31)
      - 字符转数字：
        - 此数字的 ASCII 码  与   0 的 ASCII 码 相减即可；
      - 数字拼接：
        - 若从左向右遍历数字，设当前位字符为 c ，当前位数字为 x ，数字结果为 res ，则数字拼接`res=10×res+x`
      - 越界判断：
        - `bndry=2147483647//10=214748364`
        - 每轮数字拼接前，判断 res 在此轮拼接后是否超过 2147483647
        - 如果`res>bndry`, 则乘以10必越界;
        - 如果`res==bndry，且x>7`，则拼接后的末尾为8或大于8，返回INT的边界值
          - 注意：关于MIN边界，这里无论是 = '8' , 还是 >'8', 返回的都是MIN，相当于讨了个巧
        - ![2](https://github.com/anliux/PracticePool/assets/32152091/5bb6e70d-7ca1-4824-88d1-5c2c432bc75b)
    - 非数字字符：遇到首个非数字的字符时，应立即返回
  - 算法：
    - 去掉两侧空格，并转换为字符数组
    - 判空：当字符数组长度为0时，结束，返回0
    - 初始化参数：结果res，越界边界bndry, 符号位sign，遍历开始索引i
    - 符号位：为`-`则sign=-1，否则若不为`+`, 则从i=0开始遍历(即判为非数字并直接返回)
    - 循环：从i开始遍历字符数组
      - 若不在字符数组0-9之间， 直接break跳出循环
      - 越界预判：
        - 如果res>bndry，则乘以10必越界；
        - 如果res==bndry，且x>7，则拼接后的末尾为8或大于8，返回INT的边界值
      - 拼接：`res = res*10 + (c[j]-'0');` 
    - 返回结果：`return sign * res`
  - 注意点：
    - length：字符串长度.length()和数组长度.length
      - 数组的length是属性 
    - java的最值：Integer.MAX_VALUE; Integer.MIN_VALUE; Short.MAX_VALUE;...
...
- ## 题目描述
  ...
  - 例如，给定如下二叉搜索树:  root = [6,2,8,0,4,7,9,null,null,3,5]
   - ![图片](https://github.com/anliux/PracticePool/assets/32152091/039c48b4-d0d0-46e0-977f-448d14c9c48b)
  - 示例 1:
...
- ## 解题思路
  - 概念及性质解析
    ...
    - 最近公共祖先：
      ...
      - 即假设root和p,q中间还隔着一层，但是p和q分别在root的左右两侧，此时root为**最近**的公共节点
    - 满足最近公共祖先的三种情况：
...
- ## 题目描述
  ...
  - 例如，给定如下二叉树:  root = [3,5,1,6,2,0,8,null,null,7,4]
    - ![图片](https://github.com/anliux/PracticePool/assets/32152091/80ef1723-e1ba-45b6-a63a-18ee8a89ffde)
  - 示例 1:
...
- ## 解题思路
  - 分析：
    - 思路：
      - 递归，根据不同的情况调用子树
      - DFS: 深度对比，判断是否有节点与p或q相等；如果没有则继续深入，调用左右子树，并对子节点进行判断 
    - 三种情况：
      * 1、p q 一个在左子树 一个在右子树 那么当前节点即是最近公共祖先
      * 2、p q 都在左子树 
      * 3、p q 都在右子树
      * 此外还有一种情况是找不到最近公共祖先，返回null
  - 算法：
    - 判空：如果root为null，则直接返回
    - 判断root是否等于p或q：如果是则终止递归，并返回root
    - 初始化左右子节点，并递归调用函数进行赋值
    - 对root的左右子节点进行判断，分三种情况：
      - 如果左右子节点都不为空：即分别在左右子树中找到了p或q
        - 符合最近公共祖先的条件，返回root 
      - 如果左子树不为null，则p和q都在左子树中：
        - 返回left 
      - 如果右子树不为null，则p和q都在右子树中：
        - 返回right 
      - 否则，即左右子树都为null：
        - 返回null 
