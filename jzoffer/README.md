...
* [63. 股票的最大利润](#63-股票的最大利润)：暴力法；一次遍历；dp
...
- ## 解题思路
  ...
  - 一次遍历：
    ...
    - 算法：
      ...
      - 定义两个最值：当前最小值min和最大差值maxDiff, 并初始化；
      - 遍历：先小后大 
        - 每次更新min：当[i-1]值更小时，更新min
        - 对比当前差值与最大差值：比较[i]-min与maxDiff的大小并更新maxDiff；
      - 返回maxDiff。
    - 注意：
      - min：初始化是第一个元素，因此循环中应从第二个元素开始，而i是从第三个元素开始，注意[i-1]；
      - maxDiff：初始化应为0，而不是[1]-[0], 防止出现负数
